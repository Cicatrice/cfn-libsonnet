(import '../../../extensions/attributes.libsonnet') + {
  ...
  new(errorOnEmptyProp=true): { Type: 'AWS::AppStream::ApplicationFleetAssociation', Properties: { ApplicationArn: if errorOnEmptyProp then (error 'You need to define ApplicationArn properties for AWS::AppStream::ApplicationFleetAssociation resource') else null, FleetName: if errorOnEmptyProp then (error 'You need to define FleetName properties for AWS::AppStream::ApplicationFleetAssociation resource') else null } },
