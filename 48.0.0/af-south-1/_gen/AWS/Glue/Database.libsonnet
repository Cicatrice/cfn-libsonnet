(import '../../../extensions/attributes.libsonnet') + {
  ...
  new(errorOnEmptyProp=true): { Type: 'AWS::Glue::Database', Properties: { CatalogId: if errorOnEmptyProp then (error 'You need to define CatalogId properties for AWS::Glue::Database resource') else null, DatabaseInput: if errorOnEmptyProp then (error 'You need to define DatabaseInput properties for AWS::Glue::Database resource') else null } },
