(import '../../../extensions/attributes.libsonnet') + {
  ...
  new(errorOnEmptyProp=true): { Type: 'AWS::AppStream::StackFleetAssociation', Properties: { StackName: if errorOnEmptyProp then (error 'You need to define StackName properties for AWS::AppStream::StackFleetAssociation resource') else null, FleetName: if errorOnEmptyProp then (error 'You need to define FleetName properties for AWS::AppStream::StackFleetAssociation resource') else null } },
