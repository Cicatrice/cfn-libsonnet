(import '../../../extensions/attributes.libsonnet') + {
  ...
  new(errorOnEmptyProp=true): { Type: 'AWS::Greengrass::CoreDefinitionVersion', Properties: { Cores: if errorOnEmptyProp then (error 'You need to define Cores properties for AWS::Greengrass::CoreDefinitionVersion resource') else null, CoreDefinitionId: if errorOnEmptyProp then (error 'You need to define CoreDefinitionId properties for AWS::Greengrass::CoreDefinitionVersion resource') else null } },
