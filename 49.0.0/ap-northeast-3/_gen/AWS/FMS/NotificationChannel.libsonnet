(import '../../../extensions/attributes.libsonnet') + {
  ...
  new(errorOnEmptyProp=true): { Type: 'AWS::FMS::NotificationChannel', Properties: { SnsRoleName: if errorOnEmptyProp then (error 'You need to define SnsRoleName properties for AWS::FMS::NotificationChannel resource') else null, SnsTopicArn: if errorOnEmptyProp then (error 'You need to define SnsTopicArn properties for AWS::FMS::NotificationChannel resource') else null } },
