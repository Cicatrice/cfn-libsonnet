(import '../../../extensions/attributes.libsonnet') + {
  ...
  new(errorOnEmptyProp=true): { Type: 'AWS::Route53::KeySigningKey', Properties: { KeyManagementServiceArn: if errorOnEmptyProp then (error 'You need to define KeyManagementServiceArn properties for AWS::Route53::KeySigningKey resource') else null, Name: if errorOnEmptyProp then (error 'You need to define Name properties for AWS::Route53::KeySigningKey resource') else null, Status: if errorOnEmptyProp then (error 'You need to define Status properties for AWS::Route53::KeySigningKey resource') else null, HostedZoneId: if errorOnEmptyProp then (error 'You need to define HostedZoneId properties for AWS::Route53::KeySigningKey resource') else null } },
