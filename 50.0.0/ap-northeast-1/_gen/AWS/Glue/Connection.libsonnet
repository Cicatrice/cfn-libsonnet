(import '../../../extensions/attributes.libsonnet') + {
  ...
  new(errorOnEmptyProp=true): { Type: 'AWS::Glue::Connection', Properties: { ConnectionInput: if errorOnEmptyProp then (error 'You need to define ConnectionInput properties for AWS::Glue::Connection resource') else null, CatalogId: if errorOnEmptyProp then (error 'You need to define CatalogId properties for AWS::Glue::Connection resource') else null } },
